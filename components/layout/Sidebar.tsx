...
import { getUserPermissions, UserPermissions } from '@/lib/middleware/role-check';
...
export default function Sidebar() {
  ...
  const navigation = useMemo(() => {
    ...
    if (permissions.role === 'field_staff') {
      items.push(
        { name: 'My Jobs', href: '/my-jobs', icon: 'clipboard' },
        { name: 'My Reports', href: '/reports', icon: 'chart' }
      );
    } else {
      ...
      items.push(
        ...
        { name: 'Job Codes', href: '/job-codes', icon: 'list' },
        { name: 'Inventory', href: '/inventory', icon: 'box' },
        { name: 'Travel Tracking', href: '/travel-tracking', icon: 'map' }
      );
    }

    // Pro features
    if (permissions.canManageContractors) {
      items.push(
        { name: 'Contractors', href: '/contractors', icon: 'users' },
        { name: 'Submissions', href: '/contractors/submissions', icon: 'document' },
    ...
    if (permissions.canManageLicenses) {
      items.push(
        { name: 'Licenses', href: '/licenses', icon: 'ticket' },
        { name: 'Audit Trail', href: '/audit', icon: 'list' },
        { name: 'Subscription', href: '/subscription/manage', icon: 'card' },
        { name: 'Organization', href: '/settings/organization', icon: 'cog' }
      );
    }

    // Account - all users
    items.push({ name: 'Account', href: '/settings/account', icon: 'user' });
    ...
    <div className="flex flex-col h-full bg-white border-r border-gray-200">
        ...
        {navigation.map((item) => {
          ...
          return (
            <Link
              key={item.name}
              href={item.href}
              className={`flex items-center space-x-3 px-3 py-2.5 rounded-md text-sm ${
                isActive
      ...
      <div className="p-3 border-t border-gray-100">
        <button
          ...
          disabled={loggingOut}
          className="w-full flex items-center space-x-3 px-3 py-2.5 rounded-md text-sm text-gray-700 hover:bg-gray-50 disabled:opacity-50"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
